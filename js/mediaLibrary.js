// js/mediaLibrary.js
// Version 2.2.5 - Added check for addarticle.php picker mode.
var MediaLibrary = (function($){
  ...
  function loadMedia(query = "", tagFilter = "", showVariants = false) {
    $('#global-media').html('<p style="text-align:center; padding:20px;"><i class="fas fa-spinner fa-spin"></i> Loading media...</p>');
    $.ajax({
      ...
      success: function(response) {
        ...
        if (typeof response === 'string') { // Should ideally always be JSON
            try {
    ...
    if (mediaArray && Array.isArray(mediaArray) && mediaArray.length > 0) {
      mediaArray.forEach(function(mediaAsset) {
        var $item = $("<div></div>")
                      ...
                      .data("asset-data", mediaAsset); // Store the full asset data
        ...
        if (isVariant) {
            ...
            let masterTitle = mediaAsset.master_admin_title || mediaAsset.master_title || `Master ${mediaAsset.media_asset_id_for_variant}`; // Added master_title
            ...
            displayTitle = mediaAsset.admin_title || mediaAsset.title || `Image ${mediaAsset.id}`; // Fallback to Image ID
        ...
        $imgContainer.html('<img src="img/loading.gif" alt="Loading..." style="width:30px; height:30px; margin:auto; display:block;">');
        ...
        $item.on("click", function(e) { // Added event 'e'
          var clickedAssetData = $(this).data("asset-data");

          // Check if we are on addarticle.php AND if a global "picker mode" is active
          // window.currentArticleImageTarget is a global variable that addarticle.js will set
          if ($('body').find('#article-form').length > 0 && // Check if we're on addarticle page
              typeof window.currentArticleImageTarget !== 'undefined' &&
              window.currentArticleImageTarget &&
              window.currentArticleImageTarget.type &&
              typeof window.handleMediaLibrarySelectionForArticle === 'function') {

              console.log("[MediaLibrary] Clicked in 'picker mode' for addarticle.php. Handing off to handleMediaLibrarySelectionForArticle.");
              e.stopImmediatePropagation(); // Prevent other general clicks if necessary, though specific target is better
              window.handleMediaLibrarySelectionForArticle(clickedAssetData);
              return; // Exit, let addarticle.js handle the UIE opening with context
          }

          // --- Original/Default behavior for medialibrary.php page ---
          ...
          let physicalUrlForUIE = clickedAssetData.image_url;
          let uieOpenOptions = {};
          ...
          if (clickedAssetData.is_variant) {
            masterAssetDataForUIE = {
                ...
                image_url: clickedAssetData.image_url // Physical image URL is always needed for Cropper base
            };
            ...
            try {
                ...
                } else { throw new Error("Variant details are missing."); }
            } catch (parseError) {
                console.error("Error parsing variant_details from clicked variant data:", parseError, clickedAssetData.variant_details);
          ...
          UnifiedImageEditor.openEditor(
            ...
            function() { // onEditorClosedCallback
              console.log("Unified Image Editor was closed on medialibrary.php.");
            },
            uieOpenOptions
          );
        });
        ...
        physicalImg.onload = () => {
            ...
            if (isVariant && mediaAsset.variant_details_parsed) {
                ...
                try {
                    if (mediaAsset.default_crop && mediaAsset.default_crop !== "null" && String(mediaAsset.default_crop).trim() !== "") {
                        cropToUse = JSON.parse(mediaAsset.default_crop);
                    }
                    if (mediaAsset.filter_state && mediaAsset.filter_state !== "null" && String(mediaAsset.filter_state).trim() !== "") {
                        filtersToApply = JSON.parse(mediaAsset.filter_state);
                    }
                } catch (e) {
                    console.error("Error parsing default crop/filter for media library item:", mediaAsset.id, e, "Crop:", mediaAsset.default_crop, "Filter:", mediaAsset.filter_state);
                }
            }
            ...
            if (cropToUse.width <= 0 || cropToUse.height <= 0) { // Should have been caught above, but double check
                cropToUse.width = physicalImg.naturalWidth; cropToUse.height = physicalImg.naturalHeight;
                cropToUse.x = 0; cropToUse.y = 0;
            }
      ...
      $globalMedia.html("<p style='text-align:center; padding:20px;'>No media found matching your criteria.</p>");
    } else {
      $globalMedia.html("<p style='text-align:center; padding:20px;'>Received invalid media data format.</p>");
  ...
  function init(){
    ...
    // Handle immediate load on filter change
